...
/** Resolve the habit's stored category string to a CATEGORIES entry,
 *  falling back to a synthetic one that keeps the habit's own color. */
function resolveCategory(habit) {
  const match = CATEGORIES.find((c) => c.name === habit.category);
  if (match) return match;
  return { name: habit.category || CATEGORIES[0].name, color: habit.color || CATEGORIES[0].color };
}

/** Derive the frequency id from the stored value, or infer it from the schedule. */
function resolveFrequencyId(habit) {
  if (habit.frequency && FREQUENCIES.some((f) => f.id === habit.frequency)) {
    return habit.frequency;
  }
  const len = (habit.schedule || []).length;
  if (len === 7) return "daily";
  if (len === 1) return "weekly";
  return "custom";
}
...
function EditHabit() {
  ...
  // Build initial form state once from the existing habit. Hooks must run
  // unconditionally, so we compute safe defaults even when habit is missing.
  const initial = useMemo(() => {
    if (!habit) return null;
    const cat = resolveCategory(habit);
    const freqId = resolveFrequencyId(habit);
    return {
      name: habit.name || "",
      category: cat,
      frequencyId: freqId,
      customDays:
        freqId === "custom"
          ? habit.schedule && habit.schedule.length
            ? habit.schedule
            : ["Mon", "Wed", "Fri"]
          : ["Mon", "Wed", "Fri"],
      goal: habit.goal || "",
  ...
  const [frequencyId, setFrequencyId] = useState(initial?.frequencyId ?? "daily");
  const [customDays, setCustomDays] = useState(initial?.customDays ?? ["Mon", "Wed", "Fri"]);
  ...
  // Render the not-found state *after* hooks have run.
  if (!habit) {
    return (
      <div className="add-habit animate-fade-in">
        <Link to="/habits" className="habit-details__back">
        ...
        <div className="card card--padded" style={{ marginTop: 16 }}>
          ...
          <p className="card__sub">
            The habit you're trying to edit doesn't exist or was deleted.
          </p>
        </div>
      </div>
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.synced === false) {
        toast.success(`"${name.trim()}" updated — saved locally (backend unavailable).`);
      } else {
        toast.success(`"${name.trim()}" updated.`);
      }
      navigate(`/habits/${habit.id}`);
    } catch (err) {
      toast.error(err?.message || "Couldn't update habit. Try again.");
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="add-habit animate-fade-in">
      <header className="add-habit__header">
        <div>
          <h1 className="add-habit__title">Edit Habit</h1>
          <p className="add-habit__subtitle">
            Refine your goal, adjust the rhythm, and keep your momentum on track.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="add-habit__layout">
        ...
        <aside className="add-habit__preview" aria-label="Habit preview">
          ...
          <div className="motivation-card">
            ...
            <div>
              <div className="motivation-card__title">Keep refining.</div>
              <p className="motivation-card__text">
                Adjusting a habit to fit your life is how you make it stick for good.
              </p>
            </div>
          </div>
        </aside>
